...
export async function POST(req: Request) {
  try {
    ...
    const { data: tenantData, error: tErr } = await admin.from('tenants').select('id').eq('slug', tenantSlug).maybeSingle();
    if (tErr) throw tErr;
    const tenant = tenantData as TenantIdRow | null;
    ...
    const { data: gameData, error: gErr } = await admin
      ...
      .maybeSingle();
    if (gErr) throw gErr;
    const game = gameData as GameIdRow | null;
